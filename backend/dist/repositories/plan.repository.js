...
let PlanRepository = class PlanRepository extends base_supabase_repository_1.BaseSupabaseRepository {
    constructor(supabase) {
        super(supabase, 'plans', plan_model_1.Plan);
    }
    async findByName(name) {
        ...
        return data ? plan_model_1.Plan.fromJSON(data) : null;
    }
    async findByPriceRange(minPrice, maxPrice) {
        ...
        if (error)
            throw new database_error_1.DatabaseError(error.message);
        return data.map(item => plan_model_1.Plan.fromJSON(item));
    }
};
